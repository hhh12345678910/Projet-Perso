...
def _cache_vierge():
    m._ELITE_DEEP_CACHE.clear()
    m._ELITE_DEEP_REFRESHING.clear()
    yield
    m._ELITE_DEEP_CACHE.clear()
    m._ELITE_DEEP_REFRESHING.clear()
...
def _q(cle="k", label="home", odd=2.0, line=None):
    return OddQuote(event_key=cle, book=Book.ELITESPORTS, market=MarketType.H2H,
                    ...
                    fetched_at=m.datetime.now(m.timezone.utc), source_event_id="x")
...
def test_le_cycle_n_attend_jamais(monkeypatch):
    ...
    assert m._elitesports_deep_quotes("soccer") == []
...
def test_un_cache_trop_vieux_rend_RIEN_pas_des_cotes_mortes(monkeypatch):
    ...
    vieux = time.monotonic() - (m._ELITE_DEEP_MAX_AGE + 60)
    m._ELITE_DEEP_CACHE["soccer"] = (vieux, [_q()])
    assert m._elitesports_deep_quotes("soccer") == []
...
def test_un_cache_frais_est_servi(monkeypatch):
    monkeypatch.setattr(m.threading, "Thread",
                        lambda *a, **k: type("T", (), {"start": lambda s: None})())
    m._ELITE_DEEP_CACHE["soccer"] = (time.monotonic(), [_q()])
    assert len(m._elitesports_deep_quotes("soccer")) == 1
...
def test_un_seul_rafraichissement_a_la_fois(monkeypatch):
    ...
    m._elitesports_deep_quotes("soccer")
    m._elitesports_deep_quotes("soccer")
    assert len(lances) == 1
...
def test_la_route_globale_l_emporte_sur_le_cache(monkeypatch):
    ...
    monkeypatch.setattr(m.threading, "Thread",
                        lambda *a, **k: type("T", (), {"start": lambda s: None})())
    m._ELITE_DEEP_CACHE["soccer"] = (time.monotonic(),
    ...
    class Global:
        ...
        def fetch_pages(self, sport):
            yield {"_": 1}
    monkeypatch.setattr(m, "EliteSportsScraper", Global)
    monkeypatch.setattr(m, "elitesports_parse_prematch",
...
def test_le_coupe_circuit_desactive_tout(monkeypatch):
    ...
    monkeypatch.setattr(m, "_ELITE_DEEP_ENABLED", False)
    m._ELITE_DEEP_CACHE["soccer"] = (time.monotonic(), [_q()])
    assert m._elitesports_deep_quotes("soccer") == []
...
def test_le_rafraichissement_ne_leve_jamais(monkeypatch):
    ...
    m._ELITE_DEEP_REFRESHING.add("soccer")
    m._elitesports_deep_refresh("soccer")          # ne doit pas lever
    assert "soccer" not in m._ELITE_DEEP_REFRESHING, "le drapeau reste bloqué"
