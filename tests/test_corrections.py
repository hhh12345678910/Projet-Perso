...
def _open_row(odd_taken=2.30, book="unibet_be", detected=T0):
...
def test_correction_recorded_when_the_price_drops_below_what_we_took():
    ...
    obs, corr = track_corrections([_open_row(2.30)], [_quote(2.20)],
...
def test_no_correction_while_the_price_holds():
    obs, corr = track_corrections([_open_row(2.30)], [_quote(2.30)],
                                  T0 + timedelta(minutes=7))
...
def test_a_longer_price_is_not_a_correction():
    ...
    obs, corr = track_corrections([_open_row(2.30)], [_quote(2.45)], T0)
    assert corr == []
...
def test_a_missing_market_is_not_counted_as_uncorrected():
    ...
    obs, corr = track_corrections([_open_row()], [], T0 + timedelta(hours=1))
    assert obs == [] and corr == []
...
def test_another_outcome_does_not_close_our_row():
    obs, corr = track_corrections([_open_row(2.30)], [_quote(1.50, label="away")], T0)
    assert obs == [] and corr == []
...
def test_lowest_quote_of_the_cycle_wins():
    ...
    obs, corr = track_corrections([_open_row(2.30)],
...
def test_seeding_twice_does_not_restart_the_clock(tmp_path):
    ...
    s = Storage(db)
    row = (1, T0.isoformat(), None, "unibet_be", EK, "h2h", "home", None, 2.30)
...
def test_open_corrections_ignores_already_corrected(tmp_path):
    db = str(tmp_path / "v.db")
    s = Storage(db)
    now = datetime.now(timezone.utc)
    s.seed_corrections([
        (1, now.isoformat(), None, "unibet_be", EK, "h2h", "home", None, 2.3),
        (2, now.isoformat(), None, "ladbrokes_be", EK, "h2h", "away", None, 3.1),
    ])
    assert len(s.open_corrections()) == 2
    s.update_corrections([], [(now.isoformat(), 60.0, 2.2, 1)])
    remaining = s.open_corrections()
    assert [int(r["value_bet_id"]) for r in remaining] == [2]
