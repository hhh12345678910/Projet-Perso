...
from .scrapers.smarkets import SPORT_DOMAINS as SMARKETS_SPORT_DOMAINS
...
# ── la collecte ──────────────────────────────────────────────────────────
# SIX noms sur les 66 que définit `orchestration.py`. Ce n'est pas un réexport
# de confort : c'est l'interface réelle entre la boucle du daemon et la couche
# réseau. Tout le reste — les scrapers, les caches de fond, la machinerie
...
def books_coverage(
    ...
    for current_sport in [s.strip() for s in sport.split(",") if s.strip()]:
        ...
        all_q = fetch_all_parallel(current_sport, betano_file, include_file_books=True)
        if not all_q:
