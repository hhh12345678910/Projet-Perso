...
# Scooore.be (Belgian National Lottery sportsbook) runs on the Kambi platform —
# same offering API as Unibet/711/Bingoal. Only the operator code differs:
# Scooore is "bnlbe" (Belgian National Lottery). Guest-accessible; we reuse the
# Unibet/Kambi fetch + parse logic, swapping the operator and the Book tag.
BASE = "https://eu-offering-api.kambicdn.com/offering/v2018"
OFFERING = os.getenv("SCOOORE_OFFERING", "bnlbe")
...
class ScoooreScraper:
    book = Book.SCOOORE_BE
    ...
    def __enter__(self) -> "ScoooreScraper":
        return self
...
def parse_listview(data: dict) -> Iterator[OddQuote]:
    """Walk a Kambi listView payload and yield OddQuote objects tagged as
    Scooore. Identical structure to the Unibet/711/Bingoal parser."""
