...
# 711.be runs on the Kambi sportsbook platform — the exact same offering API as
# Unibet.be. Only the operator ("offering") code differs: 711 Belgium is
# "sevelevbe" (Seven Eleven BE). The public offering API is guest-accessible,
# so we reuse the Unibet/Kambi fetch + parse logic verbatim, swapping only the
# offering code and the Book tag on emitted quotes.
BASE = "https://eu-offering-api.kambicdn.com/offering/v2018"
OFFERING = os.getenv("SEVEN_ELEVEN_OFFERING", "sevelevbe")
...
def _headers() -> dict[str, str]:
    return {
        ...
        "Origin": "https://www.711sports.be",
        "Referer": "https://www.711sports.be/",
    }


class SevenElevenScraper:
    book = Book.SEVEN_ELEVEN_BE

    # Kambi sport-level group IDs are global across operators (same IDs as the
    # Unibet scraper uses) — they identify the sport tree, not the operator.
    SPORT_GROUP_IDS = {
    ...
    def __enter__(self) -> "SevenElevenScraper":
        return self
...
def parse_listview(data: dict) -> Iterator[OddQuote]:
    """Walk a Kambi listView payload and yield OddQuote objects tagged as 711.

    Identical structure to the Unibet parser — only the Book tag differs.
