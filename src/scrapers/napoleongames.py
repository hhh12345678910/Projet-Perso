...
# Napoleon Games uses the same Kambi sport group IDs as Unibet Belgium.
SPORT_GROUP_IDS = {
    "soccer": "1000093190",
    "tennis": "1000093193",
...
# Napoleon Games adds betOfferType IDs 12579 (H2H) and 12580 (TOTALS) on top
# of the standard Kambi IDs (2, 6, 11, 12).
_MARKET_BY_TYPE_ID = {
    2: MarketType.H2H,
    12579: MarketType.H2H,
    6: MarketType.TOTALS,
    12580: MarketType.TOTALS,
    11: MarketType.HANDICAP,
...
def _headers() -> dict[str, str]:
    return {
        ...
        "Origin": "https://www.napoleongames.be",
        "Referer": "https://www.napoleongames.be/",
    }


def _odd(raw: Any) -> float | None:
    """Kambi odds are decimal × 1000 (e.g. 1350 → 1.35)."""
    if raw is None:
        return None
    try:
...
class NapoleonGamesScraper:
    book = Book.NAPOLEONGAMES_BE
    ...
    def __enter__(self) -> "NapoleonGamesScraper":
        return self
...
def parse_listview(data: dict) -> Iterator[OddQuote]:
    """Walk a Kambi listView payload (Napoleon Games) and yield OddQuote objects."""
    ...
    for entry in data.get("events") or []:
        ...
        for bo in entry.get("betOffers") or []:
            ...
            for o in bo.get("outcomes") or []:
                if o.get("status") not in (None, "Open", "OPEN"):
                    continue
